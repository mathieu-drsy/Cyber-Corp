...
function setupRoutes(app, db) {
  ...
  app.post("/difficulte", (req, res) => {
    ...
    db.run(
      "INSERT INTO data (pseudo, score, difficulté, vie, etage, mdp) VALUES (?, ?, ?, ?, ?, ?)",
      ["MODIF", 420, difficultyValue, 3, 0, "test"],
      (err) => {
        if (err) {
  ...
  app.get("/ter", (req, res) => {
    // Utilisez la méthode sendFile pour renvoyer la page index.html située dans le répertoire 'view'
    res.sendFile(path.join(__dirname, "view", "terminal_test.html"));
  });
  ...
  app.get("/get-questions", (req, res) => {
    ...
    const sql =
      "SELECT * FROM questions WHERE difficulte = ? ORDER BY RANDOM() LIMIT 3"; // Sélectionner 3 questions aléatoires
